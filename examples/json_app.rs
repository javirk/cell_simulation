...
use simulation::statistics::{StatisticContainer, make_all_stats};
...
fn setup_system(state: &Setup, device: &wgpu::Device) -> CellSimulation {
    ...
    let (simulation, texture) = Simulation::from_file("saved_models/easy.json", &uniform_buffer, device).unwrap();
